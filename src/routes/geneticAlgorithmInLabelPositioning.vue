<template>
  <section class="page">
    <article>
      ...
      <ul>
        <li>每个种群可以编码成一定的数据格式，否则将很难对结果进行评估</li>
        <li>有一个确切的适应度函数来评价每个种群的好坏，方便后续的过滤和选择</li>
        <li>通过一定的交叉/突变方法，可以保留一些符合预期的特性，否则就变成了大海捞针，无法通过进化获得更好的结果</li>
      </ul>
      <h3>应用</h3>
      ...
      <ul>
        <li>尽量避免与别的标签或方框有重叠</li>
        <li>尽量靠近关联的方框</li>
        <li>在满足以上条件的前提下，优先展示在方框的左上角</li>
      </ul>
      <p>再看看上面提到的使用遗传算法的前提条件，看看是否满足：</p>
      <ul>
        <li>每个种群的数据编码：我们只需要将所有标签的坐标当成种群的属性即可</li>
        <li>适应度评价函数：我们要把上面提到的期望转换成代码实现，用来评价当前种群的优劣，理论上可行，具体实现后面再说</li>
        <li>优秀特性的保留：由于标签的位置是相互影响的，所以当我们选择保留一部分标签的坐标时，他们之间的相对位置就固定了，在后面的种群中，这一部分结果就可以得到保留，从而向预期靠近</li>
      </ul>
      <h4>适应度的评价</h4>
